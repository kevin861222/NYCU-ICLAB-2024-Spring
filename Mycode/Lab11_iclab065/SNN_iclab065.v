...
always @(posedge clk) begin
	// shift register
	if (~ker_buffer_done_flag /*& (in_valid | ~cg_en)*/) begin // no in_valid
		ker_buffer[8] <= ker;
...
always @(posedge clk) begin
	// shift register 
	if (~weight_buffer_done_flag /*& (in_valid | ~cg_en)*/) begin // no in_valid
	...
	case (global_cnt)
		...
		default: begin
			...
			// 	conv_img[2][2] = img;
				...
				conv_img[0][0] = conv_out[7:0];
				conv_img[0][1] = conv_out[7:0];
				conv_img[0][2] = conv_out[7:0];
				conv_img[1][0] = conv_out[7:0];
				conv_img[1][1] = conv_out[7:0];
				conv_img[1][2] = conv_out[7:0];
				conv_img[2][0] = conv_out[7:0];
				conv_img[2][1] = conv_out[7:0];
				conv_img[2][2] = conv_out[7:0];
			end
			// conv_img[0][0] = 0;
			// conv_img[0][1] = 0;
			// conv_img[0][2] = 0;
...
GATED_OR GATED_conv (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_ctrl_conv&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(cg_clk_conv)
);

always @(posedge cg_clk_conv) begin
